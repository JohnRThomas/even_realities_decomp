/*
 * Function: product_test_touch_key_thread
 * Entry:    0002ad94
 * Prototype: undefined __stdcall product_test_touch_key_thread(char * param_1, uint32_t param_2)
 */


void product_test_touch_key_thread(char *param_1,uint32_t param_2)

{
  char cVar1;
  cap_event_status *p_irq_stat_data;
  uint32_t uVar2;
  uint32_t uVar3;
  char *fmt;
  GlassesState *pGVar4;
  uint32_t extraout_r1;
  uint32_t extraout_r1_00;
  uint32_t *irq_status;
  uint32_t extraout_r1_01;
  uint32_t extraout_r1_02;
  uint32_t extraout_r1_03;
  uint32_t extraout_r1_04;
  uint32_t extraout_r1_05;
  uint32_t extraout_r1_06;
  uint32_t extraout_r1_07;
  uint32_t extraout_r1_08;
  uint32_t extraout_r1_09;
  uint32_t extraout_r1_10;
  uint32_t extraout_r1_11;
  uint32_t uVar5;
  uint32_t uVar6;
  uint32_t uVar7;
  uint32_t uVar8;
  k_timeout_t timeout;
  
  uVar5 = 0;
  uVar7 = 0;
  uVar6 = 0;
  uVar8 = 0;
LAB_0002adaa:
  do {
    while( true ) {
      timeout.ticks._4_4_ = 0x4000;
      timeout.ticks._0_4_ = param_2;
      z_impl_k_sem_take((k_sem *)(param_1 + 0xb0),timeout);
      if ((param_1[1] != '\x01') && (param_1[1] != '\b')) break;
      k_msleep(5000);
      param_2 = extraout_r1_00;
    }
    param_2 = extraout_r1;
  } while (-1 < (int)((uint)*(ushort *)(param_1 + 0x1070) << 0x1f));
  if (DAT_20006be0 != 0) {
    p_irq_stat_data = (cap_event_status *)sys_clock_tick_get_32();
    aw9320x_get_irq_stat(p_irq_stat_data,irq_status);
    sys_clock_tick_get_32();
    DAT_20006be0 = 0;
  }
  if (aw9320x_wearing == IN_EAR) {
    uVar3 = sys_clock_tick_get_32();
    aw9320x_diff_get((uint32_t *)(param_1 + 0x1090));
    if (30000 < (int)(uVar3 - uVar8)) {
      uVar7 = 0;
      uVar5 = uVar7;
    }
    uVar5 = uVar5 + 1;
    aw9320x_wearing = UNKNOWN;
  }
  else {
    uVar3 = uVar8;
    if (aw9320x_wearing == OUT_EAR) {
      uVar7 = sys_clock_tick_get_32();
      aw9320x_wearing = UNKNOWN;
    }
  }
  param_2 = sys_clock_tick_get_32();
  uVar2 = sys_clock_tick_get_32();
  uVar8 = uVar3;
  if (uVar5 != 1) goto LAB_0002aeae;
  if (uVar6 == 0) {
    if ((uVar7 == 0) && (param_2 = param_2 - uVar3, 30000 < (int)param_2)) {
      printk("########################turn on the mic and start to speak! holdtime %d\n");
      param_2 = extraout_r1_03;
      goto LAB_0002ae12;
    }
  }
  else {
LAB_0002ae12:
    uVar6 = 1;
  }
  goto LAB_0002ae14;
LAB_0002aeae:
  if (uVar5 == 0) goto LAB_0002adaa;
LAB_0002ae14:
  if (((int)uVar7 <= (int)uVar3) || ((int)(uVar2 - uVar7) < 0x2711)) goto LAB_0002adaa;
  cVar1 = FUN_00033e38();
  if (cVar1 == '\x01') {
    cVar1 = FUN_00033e20();
    param_2 = extraout_r1_09;
    if ((cVar1 == '\x02') &&
       (pGVar4 = __get_dashboard_state(), param_2 = extraout_r1_10,
       (byte)pGVar4->field_0x10c8 < 0xfa)) {
      pGVar4 = __get_dashboard_state();
      pGVar4->field_0x10c8 = pGVar4->field_0x10c8 + '\x01';
      param_2 = extraout_r1_11;
    }
    goto switchD_0002aed0_caseD_6;
  }
  if ((int)(undefined *)0x249f0 < (int)(uVar7 - uVar3)) {
    printk("#############################long press################################\n");
    aw9320x_diff_get((uint32_t *)(param_1 + 0x1090));
LAB_0002ae46:
    printk("aw9320x_diff_get: %d\n",*(undefined4 *)(param_1 + 0x1090));
    param_2 = extraout_r1_02;
    goto switchD_0002aed0_caseD_6;
  }
  if ((int)(uVar7 - uVar3) < (int)(undefined *)0x15f91) {
    param_2 = extraout_r1_01;
    switch(uVar5) {
    case 1:
      if (uVar6 == 0) {
        printk("#############################single click################################\n");
        goto LAB_0002ae46;
      }
      fmt = "turn off mic now\n";
      goto LAB_0002aebe;
    case 2:
      on_double_click();
      param_2 = extraout_r1_05;
      break;
    case 3:
      on_triple_click();
      param_2 = extraout_r1_06;
      break;
    case 4:
      goto switchD_0002aed0_caseD_4;
    case 5:
      FUN_00033e44();
      param_2 = extraout_r1_07;
      break;
    case 10:
      reset_all_usr_data(param_1,1);
      param_2 = extraout_r1_08;
    }
  }
  else {
    fmt = "#############################short press################################\n";
LAB_0002aebe:
    printk(fmt);
    param_2 = extraout_r1_04;
  }
switchD_0002aed0_caseD_6:
  uVar5 = 0;
  uVar6 = uVar5;
  goto LAB_0002adaa;
switchD_0002aed0_caseD_4:
  if (0 < LOG_LEVEL) {
    if (BLE_DEBUG == 0) {
      printk("%s(): sys reboot because touch event, %d click, reboot now\n\n",
             "product_test_touch_key_thread",4);
    }
    else {
      ble_printk("%s(): sys reboot because touch event, %d click, reboot now\n\n");
    }
  }
  k_msleep(500);
                    /* WARNING: Subroutine does not return */
  sys_reboot(1);
}


