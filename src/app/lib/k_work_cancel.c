/*
 * Function: k_work_cancel
 * Entry:    00076290
 * Prototype: int __stdcall k_work_cancel(k_work * work)
 */


/* exclude_from_export */

int k_work_cancel(k_work *work)

{
  ...
  if (work == (k_work *)0x0) {
  ...
  else if ((int)(work->flags << 0x17) < 0) {
    _ASSERT("ASSERTION FAIL [%s] @ %s:%d\n","!flag_test(&work->flags, K_WORK_DELAYABLE_BIT)",
            "WEST_TOPDIR/zephyr/kernel/work.c",546);
  }
  else {
    uVar4 = 0;
    ...
    if (bVar2) {
      ...
      iVar3 = cancel_async_locked(work);
