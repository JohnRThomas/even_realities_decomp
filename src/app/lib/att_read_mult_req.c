/*
 * Function: att_read_mult_req
 * Entry:    0005c5f0
 * Prototype: uint8_t __stdcall att_read_mult_req(bt_att_chan * chan, net_buf * buf)
 */


/* exclude_from_export_ai */

uint8_t att_read_mult_req(bt_att_chan *chan,net_buf *buf)

{
  ...
  if (bVar1) {
    ...
    local_28 = bt_att_create_rsp_pdu(chan,'\x0f',0);
    ...
    do {
      ...
      local_24 = '\x01';
      bt_gatt_foreach_attr(start_handle,start_handle,(bt_gatt_attr_func_t)0x85731,local_30);
    } while (local_24 == '\0');
    tx_meta_data_free((bt_att_tx_meta_data *)local_28->user_data);
    net_buf_unref(local_28);
    send_err_rsp((bt_l2cap_chan *)chan,0xe,start_handle);
  }
  else {
