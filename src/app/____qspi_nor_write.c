/*
 * Function: $_?_qspi_nor_write
 * Entry:    000641c8
 * Prototype: int __stdcall $_?_qspi_nor_write(device * dev, off_t addr, void * src, size_t size)
 */


int ____qspi_nor_write(device *dev,off_t addr,void *src,size_t size)
  ...
  if (res == (undefined1 *)0x0) {
    if (size < 4) {
      ...
      if (res == &DAT_0bad0000) {
        ...
        src = auStack_38;
LAB_0006427e:
        res = (undefined1 *)FUN_000889b0(src,size,addr);
        qspi_wait_for_completion((device *)dev->data,(nrfx_err_t)res);
      }
    }
    else {
      ...
      do {
        ...
        res = (undefined1 *)FUN_000889b0(auStack_38,n,addr);
        qspi_wait_for_completion((device *)dev->data,(nrfx_err_t)res);
