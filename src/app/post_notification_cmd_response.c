...

{
  ...
  if (((((param_1 == 0) || (param_2 == 0)) || (param_3 == 0)) || ((param_4 == 0 || (param_5 == 0))))
    ...
    if (1 < LOG_LEVEL) {
      if (BLE_DEBUG == 0) {
        printk("%s(): input parm invalid !\n","post_notification_cmd_response",param_5,0,param_1,
               param_2,param_3);
      }
      else {
  ...
  if (*(char *)(param_4 + 1) == '\x04') {
    ...
    *puVar3 = 0;
    ...
    if (2 < LOG_LEVEL) {
      puVar4 = *(undefined4 **)(param_1 + 0xffc);
      if (BLE_DEBUG == 0) {
        printk("%s(): schedule_notif#title:%s, desc:%s, ts:%d, date:%s\n",
               "post_notification_cmd_response",puVar4 + 2,(int)puVar4 + 0x19,*puVar4,
               param_2 + 0x194);
      }
      else {
    ...
    if (*(ushort *)(param_4 + 2) != 0x1b4) {
      ...
      printk("%s(): [%s-%d] invliad  message len, data_len is %d\n\n",
             ...
             (uint)*(ushort *)(param_4 + 2),iVar1,iVar5);
      return 0;
    }
    if ((*(uint *)(param_2 + 0x1b4) < 2) &&
       (iVar1 = is_on_whitelist_by_identifier((char *)(param_2 + 0x14)), iVar2 = extraout_r1_01,
       param_5 = extraout_r2_01, 1 < (iVar1 - 2U & 0xff))) {
      if (iVar1 == 1) {
  ...
  if (BLE_DEBUG == 0) {
    printk(fmt,"post_notification_cmd_response");
  }
  else {
    ble_printk(fmt);
  }
